...
export default function TeamLeaderView() {
  ...
  const pendingProjects = projects.filter(p => p.status !== 'Done' && p.status !== 'Pending by Customer').length;
  const completedProjects = projects.filter(p => p.status === 'Done').length;
  const delayedProjects = projects.filter(p => p.status !== 'Done' && isDelayed(p)).length;
                  ...
                  <TableRow key={editor.id}>
                    <TableCell className="font-medium flex items-center gap-3">
                       <Avatar className="h-8 w-8">
                         <AvatarImage src={`https://api.dicebear.com/8.x/initials/svg?seed=${editor.name}`} alt={editor.name} />
